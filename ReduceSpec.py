...
def Read_List( lst ):
    # This fucntion reads a list of images and decomposes them into a python
    # list of image names. 
    lst = lst.strip('@')
...
def SigClip(data_set, lo_sig, hi_sig):
    ...
    avg = np.sum(data_set) / n
    Sq_Diviations = [ (value-avg)**2 for value in data_set ]
    St_Diviation = np.sqrt( np.sum(Sq_Diviations)/n ) 
    min_val = avg - lo_sig*St_Diviation
    max_val = avg + hi_sig*St_Diviation
    cliped_data = []
    for val in data_set:
...
def Norm_Flat( flat ):
    ...
    # Read Data, take average and devide # 
    ...
    # Copy Header, write chages, and write file #
...
#   image (Output_name).
    ...
    print "\n====================\n" 
    # If "im_list" is a list, (ie: "@list") read and decompose it 
    # into a python list of image names.  
    if '@' in  im_list:
        im_list = Read_List(im_list)
    ...
    print "Sigma Cliping Factors (low, high): (%s, %s)" % (lo_sig, hi_sig)
...
# ===========================================================================            
# ===========================================================================
# This peice makes it work from the comand line. 
# All the main functions are called here. 
...
if __name__ == "__main__":
    ...
    # Unpack list from command line and take thier names # 
    scriptname = args[0]
    zero_list = Read_List( args[1] )
    name_zero = args[1].lstrip("@list") + '.fits'
    flat_list = Read_List( args[2] )
    name_flat = args[2].lstrip("@list") + '.fits'
    spec_list = Read_List( args[3] ) 
    name_spec = args[3].lstrip("@list") + '.fits'    
    
    # Default Values for special commands if None are Given #   
    ...
    comb_zero = imcombine(zero_list, name_zero, method, lo_sig= lo_sig, 
                          hi_sig= hi_sig, overwrite= overwrite)
    b_flat_list = Bias_Subtract(flat_list, comb_zero )
    comb_flat = imcombine(b_flat_list, 'b.'+name_flat, method, lo_sig= lo_sig, 
                          hi_sig= hi_sig, overwrite= overwrite)
    nb_flat = Norm_Flat(comb_flat)
    b_spec_list = Bias_Subtract(spec_list, comb_zero)
    comb_b_spec = imcombine(b_spec_list, 'b.'+name_spec, method, 
                            lo_sig= lo_sig, hi_sig= hi_sig, overwrite= overwrite)
    fb_spec = Flat_Field(comb_b_spec, nb_flat)
    print "\n====================\n"
    print "\nDone. Ready for Apeture Extraction.\n"

# ==================================================================    
# This portion is to test in Spyder.
# Need to comment out comand line piece above.
    
#im_list = ['0315.J0818_series.fits', '0316.J0818_series.fits']
#          
#comb_im = imcombine(im_list, 'test1.fits',  method='median', overwrite='True')
#print comb_im 
