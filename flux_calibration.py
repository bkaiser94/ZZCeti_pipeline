...
def extinction_correction(lams, flux, airmass):
    ...
    # wavelength-dependent extinction coefficients from Gemini
    # https://www.gemini.edu/sciops/telescopes-and-sites/observing-condition-constraints/extinction
    gem_lams = np.array([310.0, 320.0, 340.0, 360.0, 380.0, 400.0, 450.0, 500.0, 550.0, 600.0, 650.0, 700.0, 800.0, 900.0])*10.0
    gem_ext = np.array([1.37, 0.82, 0.51, 0.37, 0.30, 0.25, 0.17, 0.13, 0.12, 0.11, 0.11, 0.10, 0.07, 0.05])

    spline_fit = interp1d(gem_lams, gem_ext, kind='cubic')
    ...
    '''
    xx = np.linspace(np.min(gem_lams), np.max(gem_lams), 1000)
    yy = spline_fit(xx)
    plt.figure()
    plt.scatter(gem_lams, gem_ext)
    plt.scatter(lams, a_lambda)
    plt.plot(xx, yy)
    ...
    plt.xlabel('Wavelength')
    plt.ylabel('Flux')
    plt.title('Extinction Corrected Spectrum')
    plt.show()
    '''
...
def flux_calibrate_now(stdlist,fluxlist,speclist,extinct_correct=False):
    ...
    for stdspecfile in standards:
        ...
        obs_spectra,airmass,exptime,dispersion = st.readspectrum(stdspecfile) #This is an object containing opfarr,farr,sky,sigma,warr
    ...
    while avocado < length:
        ...
        #np.savetxt('response_g138-31_no_extinction.txt',np.transpose([WD_spectra1.warr,(exptime1 * dispersion1 * 10.**(sens_wave1/2.5)),WD_spectra2.warr,(exptime2 * dispersion2 * 10.**(sens_wave2/2.5))]))
        #exit()
...
if __name__ == '__main__':
    ...
    #extinct_correction = True
    #if len(sys.argv) == 5:
    #    script, stdlist, fluxlist, speclist,extinct_correction = sys.argv
    #    print extinct_correction
